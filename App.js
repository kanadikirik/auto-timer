...
export default class App extends Component {
  ...
  _handleUpright = (z) => {
    if (z > 0.9 || z < -0.9) {
      if (!this.state.isTimerActive) this._startTimer();
  ...
  _saveTimerData = async () => {
    ...
    addToHistory(timerData)
      .then(() => {
        this.createMessage(
          "Timer data added to history successfullyy",
          STATUS.SUCCESS
        );
      })
      .catch(() =>
        this.createMessage(
          "Error while adding timer data to history",
          STATUS.ERROR
        )
      );
  };
  ...
  _handleHistoryVisible = () =>
    this.setState({ isHistoryVisible: !this.state.isHistoryVisible });

  render() {
    let { status, darkMode, isTimerActive, second, isHistoryVisible } =
      this.state;
    ...
    return (
      <View
        style={[styles.container, darkMode ? STYLE.BG_DARK : STYLE.BG_WHITE]}
      >
        <Info ref={this.info} />
        <View
          style={[
            {
              ...
              paddingTop: this.height * 0.05,
              paddingHorizontal: this.width * 0.05,
            },
          ]}
        >
          ...
          <TouchableOpacity
            onPress={this._handleDarkMode}
            style={[
              darkMode ? STYLE.BG_WHITE : STYLE.BG_DARK,
              { paddingHorizontal: 5, paddingVertical: 3, borderRadius: 5 },
            ]}
          >
            <Text style={[darkMode ? STYLE.DARK : STYLE.WHITE, STYLE.BOLD]}>
              {darkMode ? "Light mode" : "Dark mode"}
            </Text>
          </TouchableOpacity>
        </View>
        <View style={{ paddingHorizontal: this.height * 0.025 }}>
          <Text style={[styles.timeText, darkMode ? STYLE.WHITE : STYLE.DARK]}>
          ...
          <Text style={[darkMode ? STYLE.WHITE : STYLE.DARK, STYLE.TA_CENTER]}>
            {isTimerActive ? (
              ...
              <Text>
                Put the phone to the <Text style={STYLE.BOLD}>straight</Text>{" "}
                surface for <Text style={STYLE.BOLD}>start</Text> the timer
              </Text>
            )}
          </Text>
        </View>
        <View
          style={[
            styles.menu,
            STYLE.SPACE(this.width, this.height),
            { width: this.width },
          ]}
        >
          <TouchableOpacity onPress={this._handleHistoryVisible}>
            <Feather
              name="clock"
              size={28}
              style={[STYLE.BLUE, styles.iconLeft]}
            />
          </TouchableOpacity>
          <TouchableOpacity onPress={this._saveTimerData}>
            <Feather
              name="bookmark"
              size={28}
              style={[STYLE.GREEN, styles.iconLeft]}
            />
          </TouchableOpacity>
          <TouchableOpacity onPress={this._resetTimer}>
            <Feather
              name="trash"
              size={28}
              style={[STYLE.RED, styles.iconLeft]}
            />
          </TouchableOpacity>
        </View>
